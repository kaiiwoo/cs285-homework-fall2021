...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(
        ...
        if itr == 0:
            ...
            with open(load_initial_expertdata, 'rb') as f:
                loaded_data = pickle.load(f) #전문가 정책으로 얻은 trajectory들. type dictionary / 전문가 정책으로 미리 얻어논 애들
            paths = loaded_data
            ...
            paths, envsteps_this_batch = utils.sample_trajectories(self.env, self.agent.actor, batch_size, #포인트 갯수니까 맞는 것
                                                                max_path_length=self.params['ep_len'], render=False)
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for path in paths:
            for t, ob in enumerate(path['observation']):
                path['action'][t] = expert_policy.get_action(ob)
        return paths
